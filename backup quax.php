<?php
// /public_html/x/player_quax.php

require_once 'cdn_helper.php';

$cdnBaseUrl = 'https://qu.ax/'; // URL CDN untuk Qu.ax
$fileUrl = '';
$videoTitle = 'File Not Found';
$fileExists = false;
...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    if (checkRemoteFileExists($potentialFileUrl)) {
        ...
        if (in_array($fileExtension, $videoExtensions)) {
            ...
            header('Location: ' . $potentialFileUrl);
            exit();
        }
    }
}
...
if (!$fileExists) {
    ...
    die("<h1>404 Not Found</h1><p>The file from Qu.ax does not exist.</p>");
}
...
<?php
// /public_html/x/player_pomf.php

// Panggil file helper yang berisi fungsi cURL kita
require_once 'cdn_helper.php';

$cdnBaseUrl = 'https://pomf2.lain.la/f/'; // URL CDN untuk Pomf
$fileUrl = '';
$videoTitle = 'File Not Found';
$fileExists = false;

// Periksa apakah ada 'id' yang dikirim dari .htaccess
if (isset($_GET['id']) && !empty($_GET['id'])) {
    // Ambil nama file dari URL dan bersihkan untuk keamanan
    $fileName = basename($_GET['id']);
    
    // Buat URL lengkap ke CDN
    $potentialFileUrl = $cdnBaseUrl . rawurlencode($fileName);
    
    // Gunakan fungsi cURL untuk memeriksa apakah file ada
    if (checkRemoteFileExists($potentialFileUrl)) {
        $fileExists = true;
        
        // Tentukan ekstensi file
        $fileExtension = strtolower(pathinfo($fileName, PATHINFO_EXTENSION));
        // Daftar ekstensi yang akan dianggap sebagai video
        $videoExtensions = ['mp4', 'webm', 'ogg', 'mov'];

        // Periksa apakah file adalah video
        if (in_array($fileExtension, $videoExtensions)) {
            // Jika video, siapkan variabel untuk ditampilkan di halaman HTML
            $fileUrl = $potentialFileUrl;
            $videoTitle = htmlspecialchars(pathinfo($fileName, PATHINFO_FILENAME));
        } else {
            // Jika BUKAN video (gambar, audio, dll), lakukan redirect langsung
            header('Location: ' . $potentialFileUrl);
...
if (!$fileExists) {
    http_response_code(404);
    die("<h1>404 Not Found</h1><p>The file from Pomf does not exist.</p>");
}
